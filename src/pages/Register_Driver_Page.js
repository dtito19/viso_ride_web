...
function RegisterDriverContent() {
  ...
  const [isNidaValid, setIsNidaValid] = useState({
    isNidaValid: false,
    message: "",
  });
  ...
  const [isPlateNumValid, setIsPlateNumValid] = useState({
    isPlateNumValid: false,
    message: "",
  });
  ...
  const [operatorId, setOperatorId] = useState(
    JSON.parse(localStorage.getItem("OperatorId"))
  );
  ...
  useEffect(() => {
    if (station) {
      ...
      if (window.confirm("No station data, press Ok to add station data")) {
        navigate("/registerStation");
      }
    }
  }, [station, navigate]);
  ...
  const handleSubmit = (event) => {
    ...
    if (drivers.length < 1) {
      alert("Tafadhali jaza taarifa za dereva");
    } else {
      SubmitFormData();
  ...
  const handleSave = (event) => {
    ...
    if (
      ...
      !isNidaValid.isNidaValid &&
      !isLicenseValid.isLicenseValid &&
      !isPlateNumValid.isPlateNumValid
    ) {
      const driver = {
        id: Math.random() * 10,
      ...
      localStorage.setItem(
        "driverList",
        JSON.stringify([...driverList, driver])
      );
  ...
  const onChange = (event) => {
    if (event.target.name === "phone_number") {
      if (!validatePhoneNumber(event.target.value)) {
        setIsPhoneValid({
          isPhoneValid: true,
          message: "Ingiza Namba sahihi ya simu",
        });
    ...
    if (event.target.name === "license_number") {
      if (!validatelicense_number(event.target.value)) {
        setIsLicenseValid({
          isLicenseValid: true,
          message: "Ingiza Namba sahihi ya leseni",
        });
      } else {
    ...
    if (event.target.name === "nida_number") {
      if (!validateNationalID(event.target.value)) {
        setIsNidaValid({
          isNidaValid: true,
          message: "Ingiza Namba sahihi ya NIDA",
        });
      } else {
    ...
    if (event.target.name === "registration") {
      if (!validatePlateNum(event)) {
        setIsPlateNumValid({
          isPlateNumValid: true,
          message: "Ingiza Namba sahihi ya pikipiki",
        });
      } else {
          ...
          <Box sx={{ display: "flex" }}>
            ...
            >
              ...
              <Container maxWidth="lg" sx={{ mt: 1, mb: 4 }}>
                <Title>Jaza Taarifa Za Dereva</Title>
                <form onSubmit={handleSave} sx={{ mt: 0, mb: 1 }}>
                  <Grid
                    container
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  >
                    ...
                    >
                      <Title>Taarifa Za Dereva Kituo Cha {station?.ward}</Title>
